...
$sql = "select Name, Address, username, password from user_account where Type ='Cơ sở sản xuất'";
...
if (isset($_GET['action']) && $_GET['action'] != "" && $_GET['action'] == 'sort') {
    $result = $db->query('SELECT Name, Address, username, password FROM user_account 
                                where Type="Cơ sở sản xuất" ORDER BY ' . $column . ' ' . $sort_order);
}
// search query
if (isset($_GET['search'])) {
    $filtervalues = $_GET['search'];
    $query = "SELECT Name, Address, username, password FROM user_account 
              WHERE Type ='Cơ sở sản xuất' and CONCAT(Name,Address,username) LIKE '%$filtervalues%' ";
